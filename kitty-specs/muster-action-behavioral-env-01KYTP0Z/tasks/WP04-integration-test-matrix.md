...
2. **A missing/invalid skills-manifest fixture would make case (b)'s "mandated negative-path" failure vacuous** (`debugger-debbie`'s finding): muster's `runBehavioralSkillCase` reads `skillDir`/`querySetPath` *before* any network call — pointing case (b)/(c) at a manifest that doesn't parse, or that references a non-existent skill dir, would report `failed`/`1` for a **fixture defect**, not the dead endpoint. **Fix**: T022-T024 must use a genuinely schema-valid skills manifest fixture (real `skillDir` + query set — reuse WP02's `tests/fixtures/skills-anchor/` assets if suitable, or build a WP04-owned equivalent under `tests/fixtures/`, since `tests/fixtures/**` is this WP's own owned glob), and case (b)'s assertion must additionally grep the report content for a connection-refused/network-error signature (not just check the aggregate `exit-code`), to prove the failure came from the dead endpoint specifically. **F-7 (WP02 review, Info, noted not fixed here)**: `tests/fixtures/**` is declared in both WP02's and this WP's `owned_files` glob, and this bullet itself invites this WP to reuse (or independently extend) `tests/fixtures/skills-anchor/`. That is a real, if narrow, crack in WP02's "the control's fixture/assertion/falsification are a single, jointly-reviewed object" guarantee — a second WP touching the same fixture glob, even by reuse rather than edit, means the object is no longer reviewed by exactly one WP's reviewer. If this WP does anything beyond read-only reuse of WP02's existing fixture files (e.g. adds new files under `tests/fixtures/`, which is expected and fine), keep WP02's own fixture files themselves untouched, and call this out explicitly to the reviewer rather than letting the co-ownership pass unremarked.
